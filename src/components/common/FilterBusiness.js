...
function FilterBuiness({data, categoryData, filterType, filterTitle,  type, setType, setGender, gender, city, setCity, categoryState, setCategoryState }) {
    ...
    <div className={style.mainFilterDiv}>
            ...
            <h1 className={style.filterTitle}>{filterTitle}</h1>
               ...
               Data?.data?.map((item, index) => (
                   ...
                   Data?.data?.map((item, index) => (
                      <div key={index} className={style.accordionDiv}>
                       <Accordion subCategory={item.categories} index={index} title={item.title} subData= {item.subtitle} filerAction = {filerAction} activeFilter = {activeFilter} />
                      </div>
                   ))
